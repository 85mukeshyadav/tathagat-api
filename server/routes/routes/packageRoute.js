...
module.exports = (app, db) => {
	...
	app.post("/createpackage", (req, res) => {
		console.log(
			"🚀 ~ file: packageRoute.js ~ line 9 ~ app.post ~ req",
			req.body
		);
		var courseId =
			typeof req.body.courseId != "unfefined" ? req.body.courseId : "";
		var subjectId =
			typeof req.body.subjectId != "unfefined" ? req.body.subjectId : "";
		var topicId =
			typeof req.body.topicId != "unfefined" ? req.body.topicId : "";
		var chapterId =
			typeof req.body.chapterId != "unfefined" ? req.body.chapterId : "";
		...
		if (courseId || subjectId || topicId || chapterId) {
			if (req.body.packageId) {
				...
				packages
					.update({ TestList }, { where: { packageId: req.body.packageId } })
					.then((package) => {
						if (!package) {
							res.json({ message: "Error in response" });
						} else {
							packages
								.findOne({ where: { packageId: req.body.packageId } })
								.then((rs) => {
			...
			console.log(
				"courseId || subjectId || topicId || chapterId not found in Package Creation "
			);
		}
	});
							...
							if (req.body.packageId != "") {
								packages
									.update(
										...
										{ where: { packageId: req.body.packageId } }
									)
									.then((package) => {
										if (!package) {
											res.json({ message: "Error in response" });
				...
				for (var i of e) {
					...
					m.questionCount =
						...
						i.TestList.map(
							(j) =>
								j.Section &&
								j.Section.map((k) => (count += k.QuestionList.length))
						);
		...
		packages
			...
			.then((e) => {
				if (!e) {
					...
					let count = 0;
					console.log(
						"🚀 ~ file: packageRoute.js ~ line 157 ~ packages.findOne ~ e",
						e
					);
					let m = {};
			...
			if (packageId != "" && studentList.length > 0) {
				for (const student of studentList) {
					...
					await userPackages
						.findOne({
							where: {
								packagePackageId: packageId,
								userEmailId: student.email_Id,
							},
						})
						.then(async (s) => {
							if (!s) {
				...
				return res
					.status(201)
					.send({ code: 201, error: "packageId or Student is missing" });
			}
		})
	);
	...
	app.post("/mypackages_old/", (req, res) => {
		...
		userPackages
			.findAll({
				where: { userEmailId: req.body.userId },
	...
	app.post(
		...
		asyncHandler(async (req, res) => {
			...
			if (packageId != "" && studentList.length > 0) {
				let stmt =
					"INSERT INTO user_packages (package_id,user_id,created_by,updated_by) VALUES ";
				var smtValues = "";
				var index = 1;
				for (const student of studentList) {
					var assignPack = [];
				...
				return res
					.status(201)
					.send({ status: 201, error: "packageId or Student is missing" });
			}
		})
	);
	...
	app.get("/getallpackagebyid/:packageId", async (req, res) => {
		let aqlQuery =
			"SELECT pkg.*, cou.courseName,sub.subjectName, chp.chapterName, tpc.topicName  FROM packages as pkg ";
		aqlQuery +=
		...
		aqlQuery +=
			"LEFT JOIN  chapters as chp ON chp.chapterId = pkg.chapterChapterId ";
	...
	app.post("/mypackages/", async (req, res) => {
		...
		pkSqlQuery +=
			" LEFT JOIN  courses as cou ON cou.courseId = pkg.courseCourseId ";
		pkSqlQuery +=
			" LEFT JOIN  subjects as sub ON sub.subjectId = pkg.subjectId ";
		pkSqlQuery +=
			" LEFT JOIN  chapters as chp ON chp.chapterId = pkg.chapterChapterId ";
		...
		sqlQuery +=
			" INNER JOIN packages as pkg ON pkg.packageId = up.packagePackageId AND pkg.packagePrice>0";
		sqlQuery +=
			" LEFT JOIN  courses as cou ON cou.courseId = pkg.courseCourseId ";
	...
	app.get("/getPackageDetails/:packageId", async (req, res) => {
		...
		let testCount = await getMyTestCount(
		...
		if (data.pckDetails) {
			...
			let courseList = [
				{
					courseId: packageDetails.courseCourseId,
					courseName: packageDetails.courseName,
				},
			];
			let resultSet = {
				packageDetails,
	...
	async function getPackageDetails(packageId, userId) {
		...
		let aqlQuery =
			"SELECT pkg.*, cou.courseName,sub.subjectName, chp.chapterName, tpc.topicName  FROM packages as pkg ";
		aqlQuery +=
			"INNER JOIN  courses as cou ON cou.courseId = pkg.courseCourseId ";
		aqlQuery += "LEFT JOIN  subjects as sub ON sub.subjectId = pkg.subjectId ";
		aqlQuery +=
			"LEFT JOIN  chapters as chp ON chp.chapterId = pkg.chapterChapterId ";
		aqlQuery += "LEFT JOIN  topics as tpc ON tpc.topicId = pkg.topicId ";
		...
		if (result) {
			...
			let sqlQuery =
				"SELECT tst.TestTitle, tst.Test_Id, tst.subjectId, cou.courseName,sub.subjectName, chp.chapterName, tpc.topicName, testAtm.attempt_id FROM Test as tst ";
			sqlQuery +=
				" INNER JOIN  courses as cou ON cou.courseId = tst.courseCourseId ";
			sqlQuery +=
				" LEFT JOIN  subjects as sub ON sub.subjectId = tst.subjectId ";
			sqlQuery +=
				" LEFT JOIN  chapters as chp ON chp.chapterId = tst.chapterChapterId ";
			sqlQuery += " LEFT JOIN  topics as tpc ON tpc.topicId = tst.topicId ";
			sqlQuery +=
				" LEFT JOIN  test_attempted as testAtm ON testAtm.testId = tst.Test_Id AND testAtm.userId = '" +
				userId +
				"' AND testAtm.packageId = '" +
				packageId +
				"'";
			...
			let testResult = await sequelize.query(sqlQuery, {
				type: sequelize.QueryTypes.SELECT,
			});
			let subId = [];
	...
	async function getTestList(params) {
		...
		if (packageId != "") {
			...
			if (packageTestList.length > 0) {
				...
				if (userId != "") {
					sqlQuery +=
						" LEFT JOIN test_attempted as ta ON ta.testId = t.Test_Id AND ta.userId = '" +
						userId +
						"'";
				}

				sqlQuery += " WHERE t.examLevel = " + examLevel;
				if (courseId) sqlQuery += " AND t.courseCourseId = '" + courseId + "'";
				if (subjectId) sqlQuery += " AND t.subjectId = '" + subjectId + "'";
				if (chapterId)
					sqlQuery += " AND t.chapterChapterId = '" + chapterId + "'";
				if (topicId) sqlQuery += " AND t.topicId = '" + topicId + "'";

				sqlQuery += " GROUP BY t.Test_Id ";
				sqlQuery += " ORDER BY sorting_order ASC";
				console.log("sqlQuery ------- : ", sqlQuery);
				let testArr = await sequelize.query(sqlQuery, {
					type: sequelize.QueryTypes.SELECT,
				});
				if (testArr.length > 0) {
					for (test of testArr) {
	...
	async function getMyTestCount(packageId, userId) {
		const { sequelize } = db;
		let aqlQuery =
			"SELECT count(DISTINCT testId) AS myTotalTest FROM test_attempted where packageId ='" +
			packageId +
			"' AND userId ='" +
			userId +
			"'";
