...
module.exports = (app, db) => {
	...
	app.post("/createtest", (req, res) => {
		console.log(
			"🚀 ~ file: testoRoute.js ~ line 76 ~ app.post ~ req",
			req.body,
			req.body.TestTitle
		);
		Test.create({
	...
	app.get("/alltest/", async (req, res) => {
		...
		let sqlQuery =
			" SELECT `Test_Id` as `TestId`, `TestTitle`, `exam_type`, `examLevel`, `created_at`, `updated_at`, `SectionRule`, `totaltime`, `courseCourseId`, `subjectId`, `topicId`, `chapterChapterId`, `packagePackageId`, `sorting_order` ";
		sqlQuery +=
			', JSON_REPLACE(json_unquote(Section), "$[0].QuestionList", "[]", "$[1].QuestionList", "[]", "$[2].QuestionList", "[]") as `Section` ';
		sqlQuery += " FROM Test ";
		sqlQuery += " WHERE 1=1 ";
			...
			if (typeof postData.userId != "undefined") {
				...
				if (typeof postData.attempt_id != "undefined") {
					if (postData.attempt_id != "") {
						testAttempted
							.update(finalResult, {
								where: { attempt_id: postData.attempt_id },
							})
							.then((s) => {
								if (s) {
									let wahereClouse = {
						...
						if (s && typeof s.testResult != "undefined") {
							...
							for (var secData of sectionArr) {
								...
								for (var que of secData.question) {
									...
									if (que.answerStatus == "C") {
										avgCorrentSpentTime =
											avgCorrentSpentTime + parseInt(que.timeTaken);
									}
								}
								console.log(avgSpentTime);
								avgSpentTime = avgSpentTime / secData.question.length;
								avgCorrentSpentTime =
									avgCorrentSpentTime / secData.correctAnswers;
								s.testResult.section[secIndex]["avgSpentTime"] = avgSpentTime;
								s.testResult.section[secIndex]["avgCorrentSpentTime"] =
									avgCorrentSpentTime;
								secIndex++;
							}
							...
							for (var section of result.section) {
								...
								for (var que of section.question) {
									result.section[sectionIndex].question[index][
										"bookmark"
									] = false;
									await QueBookmark.forEach((obj) => {
										console.log(
											obj.questionsId.toString(),
											que.questionId.toString(),
											obj.questionsId.toString() === que.questionId.toString(),
											index,
											sectionIndex
										);
										if (
	...
	async function oldgetUserTestRank(params) {
		const { sequelize } = db;
		let aqlQuery =
			'SELECT userId, JSON_EXTRACT(testResult, "$.netScore") as score, FIND_IN_SET( JSON_EXTRACT(testResult, "$.netScore"), ( SELECT GROUP_CONCAT( JSON_EXTRACT(testResult, "$.netScore") ORDER BY JSON_EXTRACT(testResult, "$.netScore") DESC ) FROM test_attempted )) AS rank FROM `test_attempted` ';
	...
	app.post("/getTestForTree", async (req, res) => {
		...
		if (req.body) {
			let body = req.body;
			let sqlQuery =
				"SELECT tst.subjectId, tst.topicId, tst.chapterChapterId, tst.packagePackageId, tst.sorting_order, tst.Test_Id, tst.created_at, tst.SectionRule, tst.totaltime, tst.courseCourseId, tst.TestTitle, tst.exam_type, tst.examLevel, tst.instructions, cou.courseName, sub.subjectName, chp.chapterName, tpc.topicName, testAtm.attempt_id, testAtm.submitted_at FROM Test as tst ";
			sqlQuery +=
				...
				testAttempted.findAll(findObj).then(async (attemptResult) => {
					...
					for (var marks = 0; marks < maximumMarks; marks = marks + 5) {
						...
						for (var i = 0; i < marksArr.length; i++) {
							if (
								marksArr[i].totalMarks >= firstrange &&
								marksArr[i].totalMarks <= lastrange
							) {
				...
				if (packageList.length > 0) {
					for (var package of packageList) {
						...
						if (packageInfo != null) {
							...
							let filter = packageInfo.TestList.filter(
								(d) => d.TestId == req.params.testId
							);
							console.log(filter);
							if (filter.length > 0) {
